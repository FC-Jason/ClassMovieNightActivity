...
const GENRES = ["All", ...MOVIES.map(m => m.genre)];

// ===== State =====
const STORAGE_KEY = "movieNightState";
...
function vote(movieId) {
  ...
  render();
  if (previous) {
    showToast(`Your vote was changed to ${movie.title}!`, "success");
  } else {
...
function renderMyVote() {
  ...
  if (!id) {
    box.innerHTML = `<div class="card"><p><strong>${currentStudent}</strong>, you haven't voted yet.</p>
      ${state.votingOpen
        ? `<button class="btn" onclick="showSection('movies')">Pick a movie</button>`
...
function renderTeacher() {
  ...
  $("not-voted-count").textContent = STUDENTS.length - voted;
